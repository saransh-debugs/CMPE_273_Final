...
import { fmtDateTime, fmtMs, fmtTokens, parseInputText, shortId } from "../utils/format";
    ...
    <div className="max-w-[1400px] mx-auto px-8 py-12">
      ...
      {openIncidents > 0 && (
        <div className="mb-6 flex items-center gap-3 hairline rounded-sm bg-cherry/10 border-cherry/40 px-4 py-3">
          ...
          <Link
            to="/incidents"
            className="ml-auto font-mono text-[11px] uppercase tracking-wider text-fg-300 hover:text-white hairline rounded-sm px-3 py-1 hover:bg-ink-700 transition-colors"
          >
            view all →
          </Link>
        </div>
      ...
      {agentId && (
        <div className="mb-6 flex items-center gap-3 hairline rounded-sm bg-cherry/10 border-cherry/40 px-4 py-3">
          ...
          <span className="font-display italic text-[18px] text-white">
            {agentId}
          </span>
          <button
            type="button"
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3 font-mono text-[11px] text-500">
          ...
          >
            <span
              className={`inline-block w-1.5 h-1.5 rounded-full mr-2 ${
                live ? "bg-sage animate-pulse" : "bg-fg-500"
              }`}
            />
            {live ? "live" : "paused"}
          </button>
      ...
      {traces.length > 0 && (
        <div className="hairline rounded-sm overflow-hidden">
          <div className="grid grid-cols-[1fr_120px_140px_120px_100px_180px] gap-4 px-5 py-3 font-mono text-[10px] uppercase tracking-[0.18em] text-500 hairline-b bg-ink-700/30">
            <div>trace</div>
          ...
          {traces.map((t, i) => {
            const task = parseInputText(t.input_text);
            return (
            <motion.div
              key={t.trace_id}
              ...
              >
                <div className="flex flex-col gap-0.5 min-w-0">
                  <div className="flex items-center gap-3 font-mono text-[12px]">
                    <span className="text-fg-300 group-hover:text-cherry-light transition-colors">
                      {shortId(t.trace_id)}
                    </span>
                    <span className="text-500">{t.trace_id.slice(8, 16)}…</span>
                  </div>
                  {task && (
                    <div className="font-mono text-[11px] text-500 truncate group-hover:text-fg-300 transition-colors" title={task}>
                      {task}
                    </div>
                  )}
                </div>
                <div className="text-right font-mono text-[12px] text-fg-100 tabular-nums">
                  {t.span_count}
                </div>
                <div className="text-right font-mono text-[12px] text-fg-100 tabular-nums">
                ...
                <div
                  className="text-right font-mono text-[11px] text-500 whitespace-nowrap"
      ...
      {hasMore && !live &&(
        <div className="mt-6 flex justify-center">
          <button
            ...
            className="hairline rounded-sm px-6 py-2 font-mono text-[11px] uppercase tracking-wider text-fg-300 hover:bg-ink-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
