...
Collector (async gRPC, port 50051)            
   │  buffers + batches inserts               
   ▼                                          
ClickHouse (raw_spans table)                  
   │                                          
Causal Engine worker                          
   │  reconstructs DAG via vector clocks      
   │  computes per-agent blame                
   ▼                                          
ClickHouse (reconstructed_traces table)       
   │                                          
FastAPI query layer (port 8000) ──▶ UI (TBD)  
```
...
| `db/`         | `docker-compose.yml`, ClickHouse schema, `init_db.py`        | shared          |
...
- [ ] Phase 6 — Load test (simulate 100s of concurrent traces)
